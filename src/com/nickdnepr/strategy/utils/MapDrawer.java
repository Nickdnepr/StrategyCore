...
public class MapDrawer {
    ...
    private static void drawMapWithMarks(UnitsMap unitsMap, LinkedHashMap<Coordinates, Pair<Character, String>> marks) {
        System.out.print("   ");
            ...
            for (int j = 0; j < unitsMap.getSurfaceMap().getWidth(); j++) {
                if (unitsMap.getSurfaceMap().getPoint(j, i) != null) {
                    if (marks.get(new Coordinates(j, i)) != null) {
                        coloredPrint(printSpaces(marks.get(new Coordinates(j, i)).getKey().toString(), xDelimiter, ' '), marks.get(new Coordinates(j, i)).getValue());
                    } else {
    ...
    public static void drawUnitOnMap(UnitsMap unitsMap, long unitId) {
        ...
        LinkedHashMap<Coordinates, Pair<Character, String>> map = new LinkedHashMap<>();
